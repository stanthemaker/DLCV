...
def sample_images():
    random_idx = random.sample(range(1, len(train_set)), config["batch_size"])
    random_images = [train_set[i] for i in random_idx]
    sample_images = torch.stack(random_images)
...
for epoch in tqdm(range(config["num_epochs"])):
    ...
    if epoch % 20 == 0:
        torch.save(
            resnet.state_dict(),
            os.path.join(config["ckpt_dir"], f"pretrain_{name}.pt"),
        )
    epoch += 1
