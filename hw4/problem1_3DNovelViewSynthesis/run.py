...
def create_new_model(
    ...
    if cfg.data.ndc:
        ...
        model = dmpigo.DirectMPIGO(
            xyz_min=xyz_min, xyz_max=xyz_max, num_voxels=num_voxels, **model_kwargs
        )
...
def scene_rep_reconstruction(
    ...
    def gather_training_rays():
        if data_dict["irregular_shape"]:
            rgb_tr_ori = [
                images[i].to("cpu" if cfg.data.load2gpu_on_the_fly else device)
                for i in i_train
            ]
        else:
        ...
        if cfg_train.ray_sampler == "in_maskcache":
            (
                ...
                viewdirs_tr,
                imsz,
            ) = dvgo.get_training_rays_in_maskcache_sampling(
            ...
            (
                ...
                viewdirs_tr,
                imsz,
            ) = dvgo.get_training_rays_flatten(
                rgb_tr_ori=rgb_tr_ori,
                train_poses=poses[i_train],
                HW=HW[i_train],
                Ks=Ks[i_train],
    ...
    if cfg_train.pervoxel_lr:
        ...
        def per_voxel_init():
            cnt = model.voxel_count_views(
                ...
                far=far,
                stepsize=cfg_model.stepsize,
                downrate=cfg_train.pervoxel_lr_downrate,
    ...
    for global_step in trange(1 + start, 1 + cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            n_rest_scales = (
                len(cfg_train.pg_scale) - cfg_train.pg_scale.index(global_step) - 1
            )
        ...
        if (
            ...
            and global_step > cfg_train.tv_after
            and global_step % cfg_train.tv_every == 0
        ):
            if cfg_train.weight_tv_density > 0:
                model.density_total_variation_add_grad(
                    cfg_train.weight_tv_density / len(rays_o),
                    global_step < cfg_train.tv_dense_before,
                )
            if cfg_train.weight_tv_k0 > 0:
                model.k0_total_variation_add_grad(
                    cfg_train.weight_tv_k0 / len(rays_o),
                    global_step < cfg_train.tv_dense_before,
                )
        ...
        if global_step % args.i_weights == 0:
            path = os.path.join(
                cfg.basedir, cfg.expname, f"{stage}_{global_step:06d}.tar"
            )
            torch.save(
                {
                    "global_step": global_step,
...
def train(args, cfg, data_dict):
    ...
    xyz_min_coarse, xyz_max_coarse = compute_bbox_by_cam_frustrm(
...
if __name__ == "__main__":
    ...
    print(len(data_dict["HW"][data_dict["i_test"]]))
    ...
    if args.export_bbox_and_cams_only:
        ...
        np.savez_compressed(
            args.export_bbox_and_cams_only,
    ...
    if args.export_coarse_only:
        ...
        with torch.no_grad():
            ...
            alpha = (
                model.activate_density(model.density.get_dense_grid())
                .squeeze()
                .cpu()
                .numpy()
            )
            rgb = (
                torch.sigmoid(model.k0.get_dense_grid())
                .squeeze()
                .permute(1, 2, 3, 0)
                .cpu()
                .numpy()
    ...
    if args.render_test or args.inference or args.render_train or args.render_video:
        if args.ft_path:
    ...
    if args.render_train:
        testsavedir = os.path.join(
            cfg.basedir, cfg.expname, f"render_train_{ckpt_name}"
        )
    ...
    if args.render_video:
        testsavedir = os.path.join(
            cfg.basedir, cfg.expname, f"render_video_{ckpt_name}"
        )
        os.makedirs(testsavedir, exist_ok=True)
        print("All results are dumped into", testsavedir)
        rgbs, depths, bgmaps = render_viewpoints(
            render_poses=data_dict["render_poses"],
            HW=data_dict["HW"][data_dict["i_test"]][[0]].repeat(
                len(data_dict["render_poses"]), 0
            ),
            Ks=data_dict["Ks"][data_dict["i_test"]][[0]].repeat(
                len(data_dict["render_poses"]), 0
            ),
        ...
        depth_vis = plt.get_cmap("rainbow")(
            1 - np.clip((depths_vis - dmin) / (dmax - dmin), 0, 1)
        ).squeeze()[..., :3]
        imageio.mimwrite(
            os.path.join(testsavedir, "video.depth.mp4"),
