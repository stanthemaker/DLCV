...
def load_data(args):
    ...
    if args.dataset_type == "llff":
        ...
        if args.ndc:
            ...
            near_clip = max(np.ndarray.min(bds) * 0.9, 0)
            _far = max(np.ndarray.max(bds) * 1.0, 0)
            near = 0
        ...
        images, poses, render_poses, hwf, K, i_split = load_blendedmvs_data(
            args.datadir
        )
        ...
        images, poses, render_poses, hwf, i_split = load_dv_data(
            scene=args.scene, basedir=args.datadir, testskip=args.testskip
        )
