...
def calculate_precision_recall(df_merged):
    ...
    df_merged["is_tp"] = np.where(
        (df_merged["label_groundtruth"] == 1) & (df_merged["label_prediction"] == 1),
        1,
        0,
    )
...
def run_evaluation(groundtruth, predictions, verbose=False, threshold=0.5):
    ...
    df_groundtruth = load_csv(
        groundtruth, column_names=["segment_id", "frame_id", "label"]
    )
    df_predictions = load_csv(
        predictions, column_names=["segment_id", "frame_id", "label", "score"]
    )
...
def compute_confusion_matrix(df_merged, thres):
    TP = df_merged[
        (df_merged["score"] >= thres).map(bool) & df_merged["label_groundtruth"]
    ]["score"].count()
    TN = df_merged[
        (df_merged["score"] < thres).map(bool) & ~df_merged["label_groundtruth"]
    ]["score"].count()
